...
struct ContentView: View {
    ...
    @State var currentlyPlaying: SystemSoundID = 0
    
    var body: some View {
        NavigationStack {
            List(systemSounds, id: \.self) { systemSoundID in
                HStack {
                    currentlyPlaying == systemSoundID ? Image(systemName: "stop.fill") : Image(systemName: "play.circle")
                    Text(String(systemSoundID))
                    Spacer()
                }
                .contentShape(Rectangle())
                .onTapGesture {
                    playSystemSound(systemSoundID)
                }
            }
            .navigationTitle("Apple System Sounds")
        }
    }
    
    func playSystemSound(_ systemSoundID: SystemSoundID) {
        if systemSoundID == self.currentlyPlaying {
            AudioServicesDisposeSystemSoundID(systemSoundID)
        } else {
            self.currentlyPlaying = systemSoundID
